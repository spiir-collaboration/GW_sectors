...
obj = SampleFile()
obj.read_hdf("default_ANN_snr.hdf")
...
# Extracting the frequencies corresponding to the maximum amplitide of FFT-ed strain for each detector from every sample.
...
#for i in range (12000):
#    h1_freq.append(np.argmax(fftpack.fft(h1[i])))
#    l1_freq.append(np.argmax(fftpack.fft(l1[i]))) 
#    v1_freq.append(np.argmax(fftpack.fft(v1[i])))
    
#f1 = fftpack.fftfreq(512)*2048

#for i in range(12000):
#    h1_index.append(f1[h1_freq[i]])
#    l1_index.append(f1[l1_freq[i]])
#    v1_index.append(f1[v1_freq[i]])
...
for i in range(12000):
    h1_l1.append(h1_index[i] - l1_index[i])
    l1_v1.append(l1_index[i] - v1_index[i])
    h1_v1.append(h1_index[i] - v1_index[i])
